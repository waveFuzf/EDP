...
$(document).ready(function () {
			...
			switch (index) {
				case 0:
					...
					$.post('./mypage', {}, function (data, textStatus, xhr) {
						...
						$(".rc table .default").text(data.sex == "0" ? "男" : "女");
	...
	function upupup(callback) {
		...
		$("#up-btn-sure").click(function (event) {
			...
			if (imgsrc) {
				// var param = $.parseJSON($(this).attr("parameter")); //必须是规范的字符串 '{"key":"value"}' ;
				var canvas = $upimg.cropper('getCroppedCanvas', {
					width: 100,
					height: 100
				});
			
				var imgdatax = canvas.toDataURL();
				imgdatax ? upimg(imgdatax) : alertinf("请选择图片！");
			}
		});
		...
		function upimg(datax) {
			$.post('./uploadAvatar', {
				...
				if (msg == "true") {
					$("img.myavatar").attr('src', datax.toString());
					$("#mypage_modal2").modal("hide");
					myInf.image = data.datax.toString();
				} else if (msg == "error") {
					addMsg("未知错误！");
				} else if (msg == "uploadError") {
					addMsg("上传失败！");
				} else if(msg == "loginBeforupload"){
					addMsg("请先登陆！");
				}
			});
		}
	...
	// position.route([120.108478, 30.220671], [120.102478, 30.230671], () => {
	// 	// alert("Sd");
	// });
	...
	$(".rc").on('click', '.funa.eva', function(event) {
		...
		console.log("评价");
		var that=this;
		console.log($(that).parent().find(".eva_id").text());
		var box = $(this).parent(".extra").parent(".entrust");
		var time = box.find(".order_date").text();
		var passtime = Math.round((new Date() - new Date(time)) / 1000 / 60);
		var imgsrc = box.find("img").attr("src");
		(() => {
			$("#modal4 .avatar").attr("src", imgsrc);
			$("#modal4 b").text(passtime);
		})();
		$("#modal4").modal({
			closable: false,
			onDeny: function() {},
			onApprove: function() {
				...
				if (eva != '' || evl != null) {
					$.post('../evalute/eva', {
						"evaluatedid":$(that).parent().find(".eva_id").text(),
						"starLevel": num,
						"evaluate": eva,
	...
	$(".rc").on('click', '.funa.showway', function(event) {
		...
		var packlng = $(this).parent().find(".lng").text();
		var packlat = $(this).parent().find(".lat").text();
		position.drawMap(sessionStorage.getItem("longitude"),sessionStorage.getItem("latitude"),()=>{
			position.route([sessionStorage.getItem("longitude"), sessionStorage.getItem("latitude")], [packlng, packlat], () => {
				$("#modal1").modal("show");
			});
		});
...
class myPageUpdate {
  ...
  static myinf(data, box) {
    var rc = `	<table>
				<tbody>
					<tr>
						<td class="header_before"><img class="myavatar" alt=""></td>
						<td class="header_after"><button type="" class="ui positive basic button" id="changeIMG">更换头像</button></td>
					</tr>
					<tr>
						<td class="con_before">昵称：</td>
						<td class="con_after">
							<div class="ui input">
					...
					<tr>
						<td class="con_before">
							<button type="" class="ui button positive sure" style="width: 100px" class="sure">确定</button>
						</td>
					</tr>
				</tbody>
			</table>`;
  ...
  static myFetch(data, box) {
    var rc = `<div class="ui card entrust">
              ...
              <div class="content">
                <div class="header">
                  <p hidden="hidden" class="odd">单号</p>
                  <span style="margin-right:20px;" class="type">类型:${data.type}</span>
                  <span style="margin-right:20px;" class="size">大小:${data.size}</span>
                  <span>取货码：<b>${data.code}</b></span>
                  <span class="right floated"><i class="trophy icon"></i>5</span>
                </div>
                <div class="meta">
                    <span class="date">${data.address}</span>
                </div>
              ...
              <div class="extra content">
                <a href="" title="" class="express_type">${data.company}</a>
                <span>截止时间:${data.takeDate}</span>
                <a  class="showway funa" style="">导航</a>
                 <p hidden="hide" class="lng">${data.longitude}</p>
                <p hidden="hide" class="lat">${data.latitude}</p>
              </div>
          </div>`;
    box.append(rc);
  }

  static waitEvaluate(data, box) {
    var rc = `<div class="ui card entrust">
              <div class="content">
                <div class="right floated meta">
                  接单时间：<span class="order_date">${data.orderdate}</span>
                </div>
                接单人: <img class="ui avatar image" src="${data.publisheruserimage}"><span>${data.publisherusername}</span>
              </div>
              <div class="content">
                <div class="header ">
                  <span style="margin-right:20px;" class="type">单号:${data.fromNum}</span>
                  <span style="margin-right:20px;" class="size">类型:${data.type}</span>
                  <span style="margin-right:20px;">信誉限制:${data.requirement}</span>
                  <span style="margin-right:20px;"class="right">奖励积分:${data.integral}</span>
                </div>
              </div>
              <div class="extra content">
                <a href="" title="" class="express_type">顺丰</a>
                <p hidden="hide" class="eva_id">${data.publisheruserId}</p>
                <p hidden="hide" class="publish_id">${data.publishId}</p>
                <a type="" class="eva funa">评价</a>
              </div>
          </div>`;
    box.append(rc);
  }
